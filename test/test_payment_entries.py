...
def _row(importe=900, empresa="CILBRAKE SRL"):
    return {"gval_fechaconfirmacion": "2026-08-10", "gval_importe": importe,
            "gval_empresaname": empresa, "gval_descripcion": "xxx"}

def _row4002(importe=900, empresa="CILBRAKE SRL"):
    return {"gval_fechaconfirmacion": "2026-08-10", "gval_importe": importe,
            "gval_empresaname": empresa, "gval_descripcion": "xxx 4002"}
...
def test_4002_in_description_changes_subaccount():
    entries = build_payment_entries([_row4002()], MAP)
    assert entries[0]["SUBCTA"] == 1120097 and entries[0]["HABER"] == 900
